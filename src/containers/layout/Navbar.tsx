...
const NavItem = ({ href, children, onClick, index, delay }: NavItemsProps) => {
  return (
    ...
    >
      <CLink
        href={href || `/#${children}`}
        className="block p-2 duration-500 hover:text-accent font-medium text-base"
        onClick={onClick}
        withPadding
      >
        {children}
...
const Navbar = () => {
  ...
  return (
    ...
    >
      <h1 className="relative text-xl font-medium capitalize font-signature text-accent group top-1">
      ...
      {(navbarCollapsed || windowWidth > md) && (
        <nav className="capitalize absolute text-sm duration-200 md:bg-transparent z-50 w-[90%] left-1/4 transform -translate-x-10 top-full h-max rounded-xl shadow-xl p-6 bg-bg-secondary md:blocks md:static md:w-auto md:left-auto md:transform-none md:top-auto md:rounded-none md:shadow-none md:p-0 md:h-auto">
